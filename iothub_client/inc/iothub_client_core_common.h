...
#define IOTHUB_CLIENT_FILE_UPLOAD_GET_DATA_RESULT_VALUES \
    ...
    *
    * @param[in]    componentName         Name of the component associated with this request.  If this is targeting the root component, this will be NULL.
    ...
    * @param[out]   response              Response generated by the callback that will be sent to IoT Hub.  This must be allocated using malloc().  
    *                                     The SDK takes ownership of the buffer on callback return and will free() it when the SDK is done with it.
    * @param[out]   responseSize          Number of bytes application is returning in response.  This must *not* include the terminating null.
